<template>
  <!-- 与 ComputerList 完全相同的结构，只是API不同 -->
  <div>
    <div class="header" style="display: flex; justify-content: space-between; margin-bottom: 20px">
      <h2>显示器管理</h2>
      <el-button type="primary" @click="handleAdd">新增显示器</el-button>
    </div>
...
import { getMonitorList, addMonitor, updateMonitor, deleteMonitor } from '@/api/monitor'
...
const handleEdit = (row) => {
  isEdit.value = true
  dialogTitle.value = '编辑显示器'
  Object.assign(formData, row)
  dialogVisible.value = true
}

const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该显示器吗？').then(async () => {
    await deleteMonitor(row.asset_tag)
...
const handleSubmit = async () => {
  ...
  if (isEdit.value) {
    await updateMonitor(formData.asset_tag, formData)
